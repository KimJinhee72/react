...
export default function LoginPage() {
  ...
  const handleSocialLogin = async (provider: 'google' | 'github' | 'kakao' | 'naver') => {
    ...
    try {
      const providerOptions: Record<string, any> = {
        ...
        kakao: { queryParams: { scope: 'profile_nickname,profile_image' } }, // 이메일 제거
      ...
      // user 객체가 존재하면 이메일 없는 경우 가상 이메일 생성
      const user = data?.user ?? null;
      if (user) {
        let userEmail = user.email;
        if (!userEmail) {
          userEmail = `${provider}_${user.id}@example.com`;
  ...
  // 이미 로그인 되어 있는지 체크 및 auth 상태 변경 감지
  useEffect(() => {
